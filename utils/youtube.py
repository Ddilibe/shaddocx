...
class YouTubeDownLoad:
    ...
    def get_data(self):
        ...
        if self.response.status_code == 200:
            ...
            print(f"[-] - Failed to retrieve the webpage. Status code: {self.response.status_code}")

    def redirect_to_download_function(self):
        print("[+] - Checking", self.soup.find('title').getText())
        if "YouTube" in self.soup.find("title").text:
            video_pattern = re.compile(r'https://www\.youtube\.com/watch\?v=([a-zA-Z0-9_-]+)')
    ...
    def single_youtube(self):
        ...
        name = f"{self.filename}.{self.file_extension}" if self.filename else f"{yt.title}.{self.file_extension}"
